...
export function BlogEditor({ posts: initialPosts, onSave }: BlogEditorProps) {
  ...
  const handleChange = (id: number, field: keyof BlogPost, value: any) => {
    ...
    if (editingPost && editingPost.id === id) {
      let updatedEditing = { ...editingPost, [field]: value };
      if (field === 'title') {
        updatedEditing.slug = generateSlug(value);
      }
      setEditingPost(updatedEditing);
    }
  };
  ...
  const removeTag = (postId: number, tagIndex: number) => {
    setPosts(posts.map(post =>
      post.id === postId ? { ...post, tags: post.tags.filter((_, i) => i !== tagIndex) } : post
    ));
    setHasChanges(true);
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          {hasChanges && (
            <div className="self-start sm:self-center">
              <motion.span
                ...
                className="text-sm text-[#D4A373] bg-[#D4A373]/10 px-3 py-1.5 rounded-lg font-medium"
              >
                ● Non sauvegardé
              </motion.span>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <motion.button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 bg-white hover:bg-[#FAF7F2] rounded-xl text-[#634832] font-medium transition-colors border border-[#634832]/10"
            >
              <Plus size={18} />
              Nouvel Article
            </motion.button>
            ...
            <motion.button
              ...
              onClick={handleSave}
              disabled={!hasChanges}
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-[#D4A373] to-[#634832] text-white rounded-xl font-semibold shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
              <Save size={18} />
              Sauvegarder
            </motion.button>
      ...
      <div className="grid gap-6">
        {posts.map((post, index) => (
          ...
          >
            ...
            <div className="p-4 border-b border-[#634832]/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-[#634832]/60">
      ...
      <AnimatePresence>
        {isEditModalOpen && editingPost && (
          ...
          >
            ...
            >
              <div className="sticky top-0 bg-white border-b border-[#634832]/10 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 z-10">
                ...
                <button
                  onClick={closeEditModal}
                  className="self-end sm:self-center p-2 hover:bg-[#FAF7F2] rounded-xl transition-colors"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                {/* Tabs - responsive */}
                <div className="flex flex-col sm:flex-row gap-3 border-b border-[#634832]/10 pb-4">
                  {[
                    ...
                    <div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <button
                          ...
                          className="w-full sm:w-auto px-6 py-3 bg-[#D4A373]/10 hover:bg-[#D4A373]/20 text-[#D4A373] rounded-xl font-medium transition-colors"
                        >
                          Ajouter
                        </button>
                      </div>
                ...
                <div className="flex justify-end pt-6 border-t border-[#634832]/10">
                  <button
                    onClick={closeEditModal}
                    className="w-full sm:w-auto px-8 py-3 bg-[#FAF7F2] hover:bg-[#D4A373]/10 text-[#634832] rounded-xl font-medium transition-colors"
                  >
                    Fermer
                  </button>
                </div>
