  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row">
        ...
        <div className="flex flex-col min-h-0 px-4 py-6">
          ...
          <div className="mt-auto pt-8 border-t border-border">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={onLogout}
              className="w-full px-4 py-3 bg-destructive/10 hover:bg-destructive/20 text-destructive rounded-lg flex items-center gap-2 text-sm font-medium transition-colors"
            >
              <LogOut size={16} />
              Déconnexion
            </motion.button>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="shrink-0 bg-card border-b border-border sticky top-0 z-30 backdrop-blur-xl bg-card/80">
          <div className="px-4 sm:px-8 py-4 flex items-center justify-between">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onExport} className="px-4 py-2 bg-muted hover:bg-muted/80 rounded-lg flex items-center gap-2 text-sm font-medium text-foreground transition-colors">
                <Download size={16} /> Exporter
              </motion.button>
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleImport} className="px-4 py-2 bg-muted hover:bg-muted/80 rounded-lg flex items-center gap-2 text-sm font-medium text-foreground transition-colors">
              ...
              <motion.button
                ...
                className={`px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-medium transition-colors ${
                  showResetConfirm ? 'bg-destructive text-destructive-foreground' : 'bg-muted hover:bg-muted/80 text-foreground'
                }`}
              >
                <RotateCcw size={16} />
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-8">
          <div className="bg-card rounded-2xl border border-border p-4 sm:p-6 min-h-[600px]">
            {activeTab === 'hero' && <HeroEditor slides={content.heroSlides} onSave={(slides) => onUpdateSection('heroSlides', slides)} />}
            {activeTab === 'tours' && <ToursEditor tours={content.bestSellers} onSave={(tours) => onUpdateSection('bestSellers', tours)} />}
            {activeTab === 'specialties' && <TourSpecialtiesEditor specialties={content.tourSpecialties} onSave={(s) => onUpdateSection('tourSpecialties', s)} />}
            {activeTab === 'reviews' && <ReviewsEditor reviews={content.reviews} onSave={(r) => onUpdateSection('reviews', r)} />}
            {activeTab === 'blog' && <BlogEditor posts={content.blogPosts} onSave={(p) => onUpdateSection('blogPosts', p)} />}
            {activeTab === 'faq' && <FAQEditor faqs={content.faqs} onSave={(f) => onUpdateSection('faqs', f)} />}
            {activeTab === 'config' && <ConfigEditor config={content.siteConfig} onSave={(c) => onUpdateSection('siteConfig', c)} />}
            {activeTab === 'videos' && (
              <VideoConfigEditor
                videos={content.videoGallery}
                config={content.siteConfig.videos}
                onSaveVideos={(v) => onUpdateSection('videoGallery', v)}
                onSaveConfig={(c) => onUpdateSection('siteConfig', { ...content.siteConfig, videos: c })}
              />
            )}
            {activeTab === 'gallery' && <GalleryEditor images={content.imageGallery || []} onSave={(i) => onUpdateSection('imageGallery', i)} />}
            {activeTab === 'headers' && <PageHeadersEditor headers={content.pageHeaders} onSave={(h) => onUpdateSection('pageHeaders', h)} />}
            {activeTab === 'about' && <AboutEditor story={content.ourStory} onSave={(s) => onUpdateSection('ourStory', s)} />}
