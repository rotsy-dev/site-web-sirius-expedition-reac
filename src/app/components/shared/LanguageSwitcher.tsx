...
export const LanguageSwitcher = () => {
  ...
  // Normaliser le code de langue (ex: 'fr-FR' -> 'fr')
  ...
  const handleLanguageChange = async (langCode: string) => {
    setIsOpen(false);
    await changeLanguageWithAuto(langCode as 'en' | 'fr' | 'de' | 'it');
  };

  return (
    <div className="relative" ref={dropdownRef}>
      {/* Bouton d'ouverture - Drapeau uniquement */}
      <button
        ...
        {isOpen && (
          ...
          >
            {languages.map((language) => {
              const isActive = normalizeLangCode(i18n.language) === language.code;
              return (
                <button
                  key={language.code}
                  onClick={() => handleLanguageChange(language.code)}
                  disabled={isTranslating}
                  className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                    isActive ? 'bg-zinc-100 dark:bg-zinc-800' : ''
                  }`}
                >
                  <FlagIcon country={language.flag} />
                  <span className="font-medium text-zinc-700 dark:text-zinc-300">{language.name}</span>
                  {isActive && (
